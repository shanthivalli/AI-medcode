...
const TextArea = React.forwardRef(({ // Add forwardRef
  value, onChange, placeholder, disabled = false, rows = 4, className = '', wrapperClassName = '', id, label, error, isExpanded = false, ...props
}, ref) => { // Receive ref
   ...
   // If expanded, render only the textarea, no wrapper
   if (isExpanded) {
     return (
       <textarea
         ref={ref}
